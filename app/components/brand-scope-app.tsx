...
function GtmBlock({ label, field, kind, draft, setDraft, readOnly }: { label: string; field: BriefJsonField | BriefTextField; kind: "matrix" | "text"; draft: BriefDTO; setDraft: (next: BriefDTO) => void; readOnly: boolean }) {
  ...
  const columnLabels:Record<string,string>={dimension:"维度",own:"本品",competitors:"竞品对比",platform:"平台",priority:"优先级",role:"角色",tier:"层级",budgetShare:"预算配比",selection:"筛选标准",hypothesis:"假设",method:"验证方法",success:"成功标准"};
  return (
      ...
      <div>
        ...
        {kind === "matrix" && columns.length > 0 && (
          <div className="matrix-preview">
            <table>
              <thead><tr>{columns.map((col) => <th key={col}>{columnLabels[col]||col}</th>)}</tr></thead>
...
function Brief() {
  ...
  return <div className="workspace"><Sidebar project={data} active="brief"/><main className="workspace-main brief-page">
    <header className="brief-head"><div><span className="section-label">{hasGtm ? "品牌 GTM 与营销简报" : "品牌营销简报"}</span><h1>{data.brandName}<br/><em>{hasGtm ? `${data.targetMarket}市场进入与增长简报` : `${data.targetMarket}品牌营销简报`}</em></h1><p>{data.category} · 更新于 {new Date(draft.updatedAt).toLocaleDateString("zh-CN")} {data.readOnly?"· 公开只读 Demo":""}</p></div><div className="brief-actions"><button className="button light" onClick={copy}>{copied ? "已复制" : "复制内容"}</button><button className="button light" onClick={() => void download()}>导出 Markdown ↓</button>{!data.readOnly&&<button className="button dark" disabled={busy} onClick={save}>{busy ? "正在保存…" : saved ? "已保存" : "保存简报"}</button>}</div></header>
