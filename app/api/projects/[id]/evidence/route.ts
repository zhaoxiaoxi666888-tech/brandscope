...
export async function POST(request:Request,{params}:{params:Promise<{id:string}>}){
 ...
 try{const [{requireOwnedProject},{prisma},{saveEvidence}]=await Promise.all([import("@/lib/project-access"),import("@/lib/prisma"),import("@/services/evidence/evidence-store")]);const {project}=await requireOwnedProject(request,id);if(!project)return Response.json({error:"项目不存在或无权访问。"},{status:404});
  ...
  let body:{url?:unknown;discover?:unknown};try{body=await request.json();}catch{return Response.json({error:"请求内容不是有效 JSON。"},{status:400});}
  if(body.discover===true){
   const evidence=(await evidenceService.collect(project)).filter(item=>item.extractionStatus==="SUCCESS").slice(0,8-count);
   if(!evidence.length)return Response.json({error:"未能自动读取到可用的公开资料，请填写品牌官网后重试。"},{status:422});
   await saveEvidence(id,evidence,8);
   return Response.json({count:evidence.length,evidence:evidence.map(item=>({id:item.sourceId,title:item.title,url:item.url}))},{status:201});
  }
