...
export async function POST(request:Request,{params}:{params:Promise<{id:string}>}) {
  ...
  try {const [{requireOwnedProject},{prisma},{completeGeneration,reserveGeneration}]=await Promise.all([import("@/lib/project-access"),import("@/lib/prisma"),import("@/lib/usage-limits")]);const {userId,project}=await requireOwnedProject(request,id);if(!project)return Response.json({error:"项目不存在或无权访问。"},{status:404});const insights=await prisma.insight.findMany({where:{projectId:id,status:"CONFIRMED"},orderBy:{position:"asc"}});if(!insights.length)return Response.json({error:"请先确认至少一条核心洞察。"},{status:400});
