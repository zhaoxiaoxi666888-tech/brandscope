...
export type EvaluationInput={projectName:string;research:ResearchOutput[];insights:Array<{content:string;status:string}>;brief:BriefOutput};
...
export function evaluateOutput(input:EvaluationInput){
 ...
 const dimensions={
  structureCompleteness:input.research.length===6&&input.insights.length>=8&&Object.values(input.brief).every(Boolean),
